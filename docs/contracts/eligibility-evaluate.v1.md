...
The authenticated UC pre-application credit comparison uses the same app-owned OpenAI
extractor first because it is an ephemeral, single-transcript demo flow and does not need the
external service hop. If that local extraction is unavailable, the service remains the fallback.
Applications still owns the final course-context assessment in both paths.
