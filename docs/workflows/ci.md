...
- **`llm-regression`:** Temporarily paused until `2026-07-24T02:52:22Z`
  (`2026-07-24 12:52:22 AEST`). After that cutoff, it automatically returns to
  running only when `check-secrets` confirms `OPENAI_API_KEY` and path filters
  match CV and/or transcript eligibility areas. Combines former
  `cv-parser-regression` and `transcript-eligibility-regression` jobs (single
  `npm ci`, conditional API startup per area).
