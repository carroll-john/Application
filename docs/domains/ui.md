...
- UC course-detail pages present current, source-verified eligibility requirements as a short semantic list that separates academic, experience, English and placement or professional conditions. Each page links to the official UC source and shows the verification date. Where UC lists an offering but does not publish course-specific criteria, the page states that UC confirmation is required instead of inventing requirements.
- The UC demo's pre-application CV review uses applicant-friendly experience groups backed by the internal occupation classification and counts overlapping dates once. A short indicative-guidance summary appears before the groups and separates the strongest pathway message, the evidence supporting it, and the Admissions-review boundary. Each group then prioritises the role type, experience counted and an explicit `Review roles` action; entry guidance is not repeated within every row. The full browse catalogue remains beneath the initial CV upload screen but is hidden while reviewing the CV, reviewing experience, and showing matched courses. The matched-course screen reuses the same experience-group, duration, direct-entry and Admissions-review language; its reviewed-experience summary is collapsed by default so course options remain prominent, while an accessible disclosure retains the detail and edit action. Applicant-facing course cards use plain role descriptions rather than internal OSCA labels. The summary strip does not repeat a matched occupation title, and its duration comes from the corresponding reviewed experience group. The prototype's internal GPA mapping is not applicant-facing work-experience copy, and potential credit remains a separate decision. Course-match cards omit the short course summary beneath the title, rank entry guidance and credit potential as High, Medium or Low confidence, and keep the home-page catalogue summary unchanged. The default Best matches view contains up to three relevant courses whose published entry pathway is supported by the reviewed CV. When no completed qualification was found, only a clearly published standalone work-experience pathway can qualify a course for Best matches; an incomplete qualification does not satisfy a completed-qualification pathway. Specialist STEM education, initial teacher education and TESOL courses require direct domain evidence from the reviewed CV rather than generic learning or education language. Their indicative credit copy uses 6 points for graduate certificates and up to 12 points for graduate diplomas and master's courses; other longer awards may retain a higher indicative ceiling, always subject to supporting evidence and faculty assessment. Expanding an experience group lets applicants include roles and review or correct the mapped occupation title, with its classification level and short mapping description shown for context; parser confidence remains hidden on the review screen. Tertiary, secondary, and professional qualifications extracted from the CV are shown as named rows only when at least one was found; the qualifications section and its empty-state copy are omitted when all three collections are empty. The review and match introductions likewise mention qualifications only when they are present. An incomplete tertiary qualification is labelled incomplete even when the parser supplies an end year; the end year alone must never produce completed copy. Contact-detail prefill is handled later in the application rather than in this review. This pattern does not apply to StudyNext or `/review`.
- Matched-course card CTAs shortlist courses until exactly three are selected. The third selection reveals one shared assessment prompt; sign-in precedes the transcript field. Before assessment, cards show entry guidance only and do not introduce course-specific credit content. Completed results are embedded in the three selected cards as side-by-side "Original" versus "After credit" duration and 2026 indicative tuition. Positive results describe the assessment generically as using work experience and previous study rather than listing extracted subject names; the matching logic remains course-specific. When the applicant explicitly starts an application, the assessed transcript and its extracted qualification data carry into the authenticated application so the overview does not ask for the same transcript again. Completed cards do not repeat the guide-only boundary already shown above the cards. A zero-credit result retains its faculty-review status without a second summary sentence beneath the comparison.
- On the synthetic Maya MBA application, Supporting Eligibility Documentation leads with the reviewed work-experience entry pathway, retains English as met from Australian study, and treats the incomplete bachelor as not meeting a completed-degree pathway. Once English is satisfied, the page does not prompt for a separate English document.
