...
- The UC demo's pre-application CV review uses applicant-friendly experience groups backed by the internal occupation classification, counts overlapping dates once, and lets applicants include roles or correct job titles on demand. Internal classification names and confidence are not applicant-facing. Tertiary, secondary, and professional qualifications extracted from the CV are shown as named rows. Contact-detail prefill is handled later in the application rather than in this review. This pattern does not apply to StudyNext or `/review`.
