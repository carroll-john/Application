...
- The invitation-only treatment journey lives at `/assessment`; control participants continue through the ordinary 33-course catalogue. In local UC development only, `/` redirects to `/assessment` and the local gateway provisions an anonymised treatment fixture so an invitation query is unnecessary. Hosted builds remain invitation-only. Its CV review uses applicant-friendly experience groups and advisory course ranking. CV/OSCA evidence never produces credit points.
