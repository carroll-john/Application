...
eligibility request still starts immediately in the background and remains the
single parser call. Starting an application reuses and, when necessary, awaits
that in-flight request before handing genuine extracted study data and the file to
the shared qualification flow; the synthetic card assessment is never used as a
parser substitute.
