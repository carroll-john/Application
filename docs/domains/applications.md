...
- The six-week UC pilot allocates activated invitations server-side to control or treatment using a stable 50/50 assignment. Both cohorts can browse all 33 UC courses. Only treatment participants see the dedicated `/assessment` journey.
- Automated numeric guidance is governed only for Master of Business Administration (Government), Master of Education, Master of Education (STEM), and Master of Education (Leadership). Rules, mappings, published caps, provenance, approvals, expiry, catalogue, and model versions are explicit and persisted. Numeric guidance remains disabled until UC approval metadata and the exact approved rules version are configured.
- Credit points are derived only from distinct, relevant transcript study evidence and are capped by the governed course configuration. CV and OSCA evidence may rank courses or explain context but cannot add credit points. Ungoverned, expired, unapproved, insufficient, or low-confidence cases return `potentialCreditPoints: null` and manual-review guidance rather than a zero-credit decision.
- Treatment participants shortlist three courses, authenticate before transcript upload, and resume a server-backed assessment session. The trusted server result includes matched transcript evidence, confidence, manual-review reasons, and exact catalogue/rules/model versions. Applicant copy always says “Up to X credit points” and requires UC confirmation; it never derives cost or course length claims.
- `beginCourseApplication` accepts only `assessmentSessionId`. The server verifies ownership and assessment state, promotes only passed-scan evidence into the application, and records partner/version context. Prefill fills blanks only; saved and applicant-entered values remain authoritative. The existing authenticated submission flow remains the final journey.
...
| `src/lib/assessment/storageAdapter.ts` | Assessment persistence contract |
| `src/lib/assessment/ucGovernance.ts` | Versioned UC credit guidance authority |
