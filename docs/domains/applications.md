...
- When the authenticated applicant explicitly starts a UC application after that comparison, extracted transcript study fields fill only blank matching qualification fields (or create one new qualification when no match exists). Existing saved values remain authoritative. The file is attached to that qualification through the shared authenticated document path. On the first hydrated qualifications-hub visit, Applications rematches the carried evidence to the single selected course and persists that course-specific assessment through `ApplicationStorageAdapter`; the three-course credit comparison is not transferred.
