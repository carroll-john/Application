...
- The synthetic demo may reveal its fixed three-course comparison after a short presentation delay while the same uploaded transcript continues through the real parser in the background. That single in-flight parser request is reused when the applicant starts an application; application handoff waits for the genuine extracted study data so the qualifications flow never substitutes the synthetic card estimate for transcript parsing.
