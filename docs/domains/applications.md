...
- The synthetic Bill Shorten UC demonstration has one deterministic credit fixture. Before assessment, its three matched education courses show credit-confidence indicators of High, Medium and Medium in display order. After an authenticated transcript is supplied and the real parser starts for the exact three-course shortlist, Master of Education (Leadership) and Master of Education (STEM) indicate 6 credit points while Graduate Certificate in Educational Leadership remains on faculty review. The fixed result is gated by the normalized Bill/William Shorten identity and exact shortlist rather than variable model wording for the known Monash dual-degree transcript. All other applicants, shortlist combinations and courses use the ordinary evidence-based assessment.
- On the UC matched-course screen, applicants shortlist exactly three courses before starting a credit comparison. The comparison requires authentication, processes one transcript ephemerally, combines extracted study evidence with the reviewed CV match, and does not create an application draft. Outside the explicitly documented synthetic demo fixture, a positive automatic estimate requires course-related transcript evidence; CV relevance may contribute only alongside that evidence. Indicative credit, duration and tuition comparisons remain separate from admission and from UC's formal faculty decision.
- The synthetic demo may reveal its fixed three-course comparison after a short presentation delay while the same uploaded transcript continues through the real parser in the background. That in-flight parser request is reused when the applicant starts an application; if the browser request itself failed before receiving a response, Start application retries it once. Application handoff still waits for genuine extracted study data so the qualifications flow never substitutes the synthetic card estimate for transcript parsing.
- When the authenticated applicant explicitly starts a UC application after that comparison, extracted transcript study fields fill only blank matching qualification fields (or create one new qualification when no match exists). The transcript-backed qualification replaces tertiary qualification suggestions from the same incoming CV handoff, while unrelated saved or user-added qualifications remain authoritative. The file is attached to that qualification through the shared authenticated document path. On the first hydrated qualifications-hub visit, Applications rematches the carried evidence to the single selected course and persists that course-specific assessment through `ApplicationStorageAdapter`; the three-course credit comparison is not transferred. A met English-proficiency check from that assessment also satisfies the generic Section 2 English evidence step.
