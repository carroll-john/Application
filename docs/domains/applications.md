...
- The UC pre-application course matcher separates course fit from entry evidence. It ranks fit from the applicant's full reviewed CV, but a course becomes a best match only when the same CV also demonstrates a published course-specific or UC postgraduate work-entry route. Current or most recent included roles carry more weight than older roles, while completed tertiary qualifications demote equivalent or lower-level study in the same field so recommendations favour additive learning.
- UC postgraduate work-entry rules live in `vendor/eligibility-rules/src/ucWorkExperienceEntry.ts` and are versioned against the UC sources verified on 2026-08-18. Course-specific experience routes are assessed first and do not depend on the central approved-course list. The general career-history and OSCA Skill Level 1/2 routes apply only to UC's published approved postgraduate courses; the current MBA code `MGM104` is tied explicitly to the published `MGM103` identifier pending source alignment. Online law courses with UC's additional GPA caveat remain review-only.
- Never convert OSCA levels or years of work into an equivalent GPA. A missing qualification on a CV is `unknown`, not proof of the checker's explicit "no prior qualification" answer, and an incomplete bachelor remains incomplete. The checker's career-history matrix can only be applied once its prior-study answer is evidenced: Diploma/Advanced Diploma/Associate Degree = two relevant or four general years; Certificate IV/Year 12 = three relevant or six general years; no prior qualification = seven relevant or ten general years. All positive outcomes remain indicative and require UC Admissions confirmation.
- On the UC matched-course screen, applicants shortlist exactly three courses before starting a credit comparison. The comparison requires authentication, processes one transcript ephemerally, combines extracted study evidence with the reviewed CV match, and does not create an application draft. A positive automatic estimate requires course-related transcript evidence; CV relevance may contribute only alongside that evidence. Applicant-facing positive-result copy describes the combined work-experience and previous-study inputs generically; it does not expose matched transcript subject names. When the authenticated applicant explicitly starts an application, extracted study fields fill only blank matching qualification fields and the transcript is attached to that qualification through the shared document layer. Existing saved values remain authoritative. The UC demo has reviewed credit profiles for `MGM104`, `MGC103`, and `ARC701`, capped respectively at 6, 6, and 3 credit points pending faculty confirmation. These are explicit demo settings, not published credit entitlements. Time comparisons use the published 2026 course duration and total credit points. Tuition comparisons use published 2026 fee data when available, distinguish full-fee tuition from CSP student contributions, and remain hidden without a reliable course cost profile. Indicative credit, time and fees remain separate from admission and from UC's formal faculty decision.
- The qualifications hub reconciles legacy deterministic transcript checks before rendering: bachelor-level study cannot appear as a met completed-qualification pathway when the paired completion check is incomplete or unconfirmed, and a passed English check removes the generic prompt to add English evidence. For the synthetic Maya MBA demo only, the exact carried CV, RMIT study and lead-role fixture restores the pre-application work-experience pathway row until the matcher result has a general persisted application contract; it must not infer that pathway for other applicants or courses.
