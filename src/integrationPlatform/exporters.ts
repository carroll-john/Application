...
import { transferPackageManifestSchemaDefaults } from "./contracts";
...
export interface UniversityExportTemplate {
  templateId: string;
  partnerId: string;
  partnerName: string;
...
export async function generateStructuredExport(input: {
  ...
  const { application, decision, template, generatedAt } = input;
  ...
  const artifact: TransferArtifact = {
    artifactId: `artifact-${sanitizeToken(decision.decisionId)}-${sanitizeToken(template.templateId)}-${template.format}`,
  ...
  const manifest: TransferPackageManifestV1 = {
    ...
    manifestId: `manifest-${sanitizeToken(decision.decisionId)}-${sanitizeToken(template.templateId)}`,
    ...
    handoff: {
      ...template.handoff,
      idempotencyKey: `${decision.decisionId}:${template.partnerId}:${template.templateId}`,
    },
  };
  ...
  return {
    ...
    artifact,
    manifest,
    formatValidation: validateStructuredExportContent(template, content),
    manifestValidation: validateTransferPackageManifest(manifest),
  };
}
